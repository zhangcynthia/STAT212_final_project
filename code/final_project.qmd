...
used_ridership_all %>% 
  ...
  summarize(Total_Riders = sum(Total_Riders,na.rm = TRUE)) %>% 
...
used_ridership_all %>% 
  group_by(nYear) %>%
  summarize(Daily_load = n()) %>% ggplot(aes(x=nYear, y= Daily_load))+geom_line()+geom_point()

used_ridership_all %>% distinct(nYear,Route) %>% group_by(nYear) %>% 
  summarize(nRoute = n()) %>% ggplot(aes(x=nYear, y= nRoute))+geom_line()+geom_point()
```
