...
education_pt1 <- st_join(stops_census_join_summ, first_schools) %>% filter(COUNTYNAME %in% c("Ramsey", "Hennepin"))

education_final <- st_join(education_pt1, second_schools) %>% filter(COUNTYNAME %in% c("Ramsey", "Hennepin"))
education_final <- education_final %>% select(tract, geometry, GISNAME, GISADDR, LOCDISTNAM, COUNTYNAME, number_of_stops)
duplicate_rows3 <- duplicated(education_final$GISADDR)
unique_education <- education_final[!duplicate_rows, ]
...
### Based on Income & Population Size
...
### Mapping the Education Sources in Twin Cities

```{r}
schools_stops %>% 
  ggplot(aes(x = number_school, y = number_of_stops))+
  stat_summary_bin(fun = "mean", bins = 10, geom = "point")+ #divides the number of schools by the number of bins, so Bin 1: 0-10 schools, Bin 2: 10:20, etc. 
  # geom_point(alpha = 0.5)+
  geom_smooth(method = "lm")+
  labs(x = "Mean Number of Schools (Pre and Post-Secondary) in Tract", y = "Mean Number of Bus Stops in Tract")+ #average number of bus stops in areas with similar numbers of schools
  theme_classic()
