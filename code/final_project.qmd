...
census_var_perc_plot <- function(var){
...
for(i in 1:2){
  print(census_var_plot(get(name[i])))
}


for(i in 3:9){
  print(census_var_perc_plot(get(name[i])))
}
...
census_point_plot <- function(var){
 # Function for plotting the linear relationship between chosen variables and the number of bus stops
  ...
  ggplot(aes(x = {{var}}, y = number_of_stops))+
    geom_point()+
    geom_smooth(method = "lm")+
    labs(y = "Number of Bus Stops in Tract")+
    theme_classic()
}

census_point_density_plot <- function(var){
...
for (i in 1:2){
  print(census_point_plot(get(name[i])))
}


for (i in 3:7){
  print(census_point_density_plot(get(name[i])))
}
